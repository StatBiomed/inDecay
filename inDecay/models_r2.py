...
class Base_del_model(pl.LightningModule):
    ...
    def __init__(self, T = 0.5,  lr: Union[float, int] = 3e-4, L1_lambda: float = 0, L2_lambda: float = 1e-9, optim_class="Adam"):
        super().__init__()
        self.save_hyperparameters()
        self.T = T
        self.lr = lr
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        def mask_and_renormalize(pred, thre=0.05):
            if isinstance(pred, torch.Tensor):
                ...
                pred= pred_y
            return pred
        cre = self.compute_Loss(mask_and_renormalize(p_pred), y)
...
class ST_DeepDecay(Base_del_model):
	...
	def __init__(self, inputsize=9, outputsize=1, hidden=[16], lr=3e-4, L1_lambda=3e-4, L2_lambda=3e-4, T=0.5):
		super().__init__(lr=lr, L1_lambda=L1_lambda, L2_lambda=L2_lambda, T=T)
		self.lr = lr
		self.T = T
...
class ST_DeepDecay_dropout(ST_DeepDecay):
	...
	def __init__(self, inputsize=9, outputsize=1, hidden=[16], lr=3e-4, L1_lambda=3e-4, L2_lambda=3e-4,T=1):
		super().__init__(inputsize=inputsize, outputsize=outputsize, hidden=hidden, lr=lr,L1_lambda=L1_lambda,L2_lambda=L2_lambda, T=1)
...
class ST_DeepDecay_mul(ST_DeepDecay):
	...
	"""
	def __init__(self, inputsize=9, outputsize=1, hidden=[16], lr=3e-4, L1_lambda=3e-4, L2_lambda=3e-4, T=1):
		super().__init__(inputsize=inputsize, outputsize=outputsize, hidden=hidden, lr=lr,L1_lambda=L1_lambda,L2_lambda=L2_lambda, T=T)
