...
def get_sanger_training(data_archive="Sanger_training"):
    ...
    def_table= os.path.join(PATH.data_dir, "Sanger_ref.csv")
...
if __name__ == "__main__":
 ...
 # if args.GPU_devices is not None:
    ...
    if not os.path.exists(f"results/{args.data_archive}_spliting.csv"):
        save_spliting(genes, kf_indeces) # create a 2-dimensional list  then to df
    else:
        # sanity check
        spliting_df = pd.read_csv(f"results/{args.data_archive}_spliting.csv")
