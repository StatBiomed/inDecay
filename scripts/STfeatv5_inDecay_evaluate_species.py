...
def get_sanger_training(data_archive):
    """
    get all the sanger sequencing samples
    """
    # find the dir
    sanger_dir = os.path.join(data_dir, data_archive)
    assert os.path.exists(sanger_dir),[sanger_dir, "the sanger sequencing data is not correctly deposited"]
...
def read_sanger_data(gene, gene_ref_lookup, data_archive='Sanger_training'):
    ...
    sanger_df_path = os.path.join(data_dir, data_archive, f"{gene}_SelfTarget.csv")
    ...
    # read indelgen
    idgen_dir = os.path.join(data_dir, data_archive, 'Indelgen_result')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--L", required=False, type=float, default=3e-4, help="the lr")
 ...
 # if args.GPU_devices is not None:
    ...
    print(f"Runing {args.data_archive} using cuda: {gpu_device}")
    
    # Temp Theta file
    date = time.strftime("%B%d")
    sanger_dir = os.path.join(data_dir, args.data_archive)
    ...
    pth_save_dir = os.path.join(PATH.pth_dir, args.modelnote)
    pth_save_path = pj(pth_save_dir, f"{exp_name}")
    ...
    model_parsms = dict(inputsize=n_features, outputsize=1,  lr=1e-3,
    ...
    Test_DS = reader.ST_dataset(genes, gene_ref_dict, 
    ...
    Y_ls = [get_identifiers(gene) for gene in genes]
    # Y_ls = process_map(get_identifiers, Test_Oligos, max_workers=8)
    Y_lookup = {o:Y_ls[i] for i,o in enumerate(genes)}
    ...
    pred_lookup = {o:predict_y[i].cpu().numpy() for i,o in enumerate(genes)}
